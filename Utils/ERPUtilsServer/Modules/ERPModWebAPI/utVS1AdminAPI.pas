...
begin
  ...
  try
    ...
    try
      VS1_Clients := TVS1_Clients.Create(Nil);
      try
        VS1_Clients.Connection := TMyDAcDataconnection.Create(VS1_Clients);
        VS1_Clients.Connection.Connection := fConn;
        //aVS1_Clients.LoadSelect('ClientName=' + quotedstr(Params.S[VS1_TAG_ClientName] ));
        VS1_Clients.LoadSelect('DatabaseName=' + QuotedStr(Params.S[VS1_TAG_DatabaseName] ));
        try
          VS1_Clients.Connection.BeginTransaction;
          try
            if VS1_Clients.Count = 0 then begin
              OnStatus(VS1_HTTP_Failed_STRCode, 'Client record not found');
              Exit;
            end;

            if not(SameText(VS1_Clients.SampleDatabaseName, Params.S[VS1_TAG_DatabaseName])) then begin
              OnStatus(VS1_HTTP_Failed_STRCode, 'Databse Doesn''t Match');
              Exit;
            end;

            if not AddClientModule(VS1_Clients, VS1_MODULE_Add_Extra_User, params) then Exit;

            VS1_Database := TVS1_Databases.Create(nil);
            try
              VS1_Database.Connection := VS1_Clients.Connection;
              VS1_Database.LoadSelect('DatabaseName=' + QuotedStr(Params.S[VS1_TAG_DatabaseName]));

              if VS1_Database.Count = 0 then begin
                OnStatus(VS1_HTTP_Failed_STRCode, 'Invalid Database');
                Exit;
              end;

              if VS1_Database.Vs1_ClientID <> VS1_Clients.Vs1ClientID then begin
                OnStatus(VS1_HTTP_Failed_STRCode, 'Client and Database doesn''t match');
                Exit;
              end;

              EmpID := ClientEmployeeID(VS1_Database, Params, msg);
              if EmpID = 0 then begin
                OnStatus(VS1_HTTP_Failed_STRCode, 'Failed to Create User / employee');
                Exit;
              end;

              VS1_ClientUsers := TVS1_ClientUsers.Create(nil);
              try
                VS1_ClientUsers.Connection := VS1_Database.Connection;
                VS1_ClientUsers.LoadSelect('EmailId=' + QuotedStr(Params.S[VS1_TAG_VS1UserName]));
                if (VS1_ClientUsers.Count <> 0) then begin
                  if SameText(VS1_ClientUsers.DatabaseName, Params.S[VS1_TAG_DatabaseName]) then begin
                ...
                VS1_ClientUsers.New;
                VS1_ClientUsers.DatabaseName   := VS1_Database.Databasename;
                VS1_ClientUsers.VS1Employeeid  := EmpID;
                VS1_ClientUsers.EmailId        := Params.S[VS1_TAG_VS1UserName];
                VS1_ClientUsers.ClientId       := VS1_Clients.ID;
                VS1_ClientUsers.Phone          := Params.O[VS1_TAG_EmployeeDetails].S[VS1_TAG_PhoneNumber];
                VS1_ClientUsers.Password       := Params.S[VS1_TAG_VS1Password];
                VS1_ClientUsers.Firstname      := Params.O[VS1_TAG_EmployeeDetails].S[VS1_TAG_FirstName];
                VS1_ClientUsers.LastName       := Params.O[VS1_TAG_EmployeeDetails].S[VS1_TAG_LastName];
                VS1_ClientUsers.Active         := True;
                VS1_ClientUsers.PostDB;

                OnStatus(VS1_HTTP_Success_STRCode, 'OK');
                VS1_Clients.connection.CommitTransaction;
          ...
          Except
            on E:Exception do begin
              VS1_Clients.connection.RollbackTransaction;
            end;
          end;
        finally
