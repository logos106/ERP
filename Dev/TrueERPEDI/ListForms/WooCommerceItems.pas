...
begin
  ...
  if not AppEnv.CompanyPrefs.WooCommerceSendProductOptions then Exit;
  // now send message to get updated productoptions
  AFields := TJsonArray.Create;
  try
    AJSON1 := JO;
    AJSON1.S['JOBSEGMENT'] := 'WOOC_SYNCH_PRODUCT_OPTIONS';
    AFields.Add(AJSON1);
    sJSON := BuildJSON('JOB', 0, 0, 0, AFields, 'TrueERP', 'WooCommerce');
    wsJobClient.WriteData(sJSON);
    Application.ProcessMessages;
