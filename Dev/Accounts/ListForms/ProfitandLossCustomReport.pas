unit ProfitandLossCustomReport;

{ Date     Version Who  What
 -------- -------- ---  --------------------------------------------------------
 11/03/23  1.00.01 Wang  Modified to use new filtering on base listing.
 }
...
uses
  ...
  wwradiogroup, GIFImg, ProfitAndLossSQL;
...
type
  TProfitandLossCustomGUI = class(TBaseListingGUI)
    ...
    qryMainLevel5: TWideStringField;
    qryMainLevel6: TWideStringField;
    qryMainLevel7: TWideStringField;
    qryMainLevel8: TWideStringField;
    qryMainLevel9: TWideStringField;
...
begin
  ...
  try
    try
      ...
      // Check if current layout exists
      fiLayoutID := 0;
      with qryLayout do begin
        SQL.Clear;
        SQL.Add('SELECT LayoutID FROM tblPnLLayouts WHERE IsCurrentLayout="T"');
        Open;

        if RecordCount > 0 then
          fiLayoutID :=  FieldByName('LayoutID').AsInteger;

        // Get max level
        SQL.Clear;
        SQL.Add('SELECT (MAX(LENGTH(Pos)) DIV 2) AS MaxLevel FROM tblPnLLayoutData3');
        SQL.Add('WHERE LayoutID=' + IntToStr(fiLayoutID));
        Open;
        fiMaxLevel := FieldByName('MaxLevel').AsInteger - 1;
        Close;
      end;

      // Populate the data to current layout
      PnlSQLObj.LayoutID := fiLayoutID;
      PopulateReportTable;

      // Remove other Level# field
      for K := qryMain.Fields.Count - 1 downto 1 do begin
        sTemp := Copy(qryMain.Fields[K].FieldName, 1, 5);
        if not SameText(sTemp, 'Level') then Continue;

        sTemp := Copy(qryMain.Fields[K].FieldName, 6, 1);
        if StrToInt(sTemp) > fiMaxLevel then
          qryMain.Fields.Remove(qryMain.Fields[K]);
      end;
