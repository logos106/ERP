...
type
  TProfitandLossGUI = class(TBaseListingGUI)
    ...
    btnCustomLayout: TDNMSpeedButton;
    ...
    procedure btnCustomLayoutClick(Sender: TObject);
...
uses FastFuncs,DNMlib, TransAccountDetailsForm, BalanceSheetForm,
  ...
  PnLLayout, PnLLayouts, ProfitandLossCustomReport, ClipBrd;
...
begin
  ...
  PnlSQLObj.PopulateReportSQL1(qryMain.SQL, msg);
...
begin
  ...
  if not False {PnlSQLObj.UseCustomLayout} then begin
...
procedure TProfitandLossGUI.btnCustomLayoutClick(Sender: TObject);
var
  custom : TProfitandLossCustomGUI;
begin
  inherited;
  try
    custom := TProfitandLossCustomGUI.Create(nil);  // TProfitLossLayoutsGUI
    custom.ShowModal;
  finally
    custom.Free;
  end;
